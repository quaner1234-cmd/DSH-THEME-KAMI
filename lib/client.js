...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var STATE_RULES = [
      ...
      '}',
      '/* 5 ── the appearance (light/dark/system) selected cube: DSH paints an',
      '   "on" cube with --dsw-alias-bg-module-platform +',
      '   --dsw-static-neutral-bluish-400, two tokens that collapse into the',
      '   page background and the unselected border here in night mode, so a',
      '   selected cube was indistinguishable from the other two. Give it the',
      '   same ink plate as the preset card: ink-tint fill, ink rule, ink label. */',
      'html body[data-dsh-kami] [class*="_themeCube"][class*="_selected"]{',
      '  background:var(--kami-ink-tint);',
      '  border-color:var(--kami-ink-ring-strong);',
      '  box-shadow:inset 0 0 0 .5px var(--kami-ink-ring-strong);',
      '  color:var(--kami-ink);',
      '}',
      'html body[data-dsh-kami] [class*="_themeCube"][class*="_selected"]:hover{',
      '  background:var(--kami-ink-tint-deep);',
      '  border-color:var(--kami-ink-ring-strong);',
      '  color:var(--kami-ink-strong);',
      '}'
